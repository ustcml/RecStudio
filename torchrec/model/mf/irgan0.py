...
class IRGAN0(basemodel.TwoTowerRecommender):

    def init_model(self, train_data):
        super().init_model(train_data)
        self.sampler.init_model(train_data)
    
    def init_parameter(self):
        super().init_parameter()
        self.sampler.init_parameter()
    ...
    def training_step(self, batch, batch_idx, optimizer_idx):
        if optimizer_idx == self.iter_idx[self.current_epoch % sum(self.n_epochs)]:
            ...
            if optimizer_idx == 1:
                loss = self.sampler.training_step(batch, self)
    ...
    def reward_for_g(self, batch, neg_item_idx, weight):
        ...
        reward = 2 * (F.sigmoid(self.score_func(query, neg_items)) - 0.5) * weight 
        return reward
    
    def configure_optimizers(self):
        opt_d = optim.Adam(self.parameters(), lr=0.001, weight_decay=self.config['regularization'])
        opt_g = optim.Adam(self.sampler.parameters(), lr=self.config['learning_rate'])
        return [opt_d, opt_g]
