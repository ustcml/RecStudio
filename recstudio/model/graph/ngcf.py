...
class NGCF(basemodel.BaseRetriever):
    ...
    def _init_model(self, train_data: MFDataset):
        ...
        for i, (input_size, output_size) in enumerate(zip(self.layers[ : -1], self.layers[1 : ])):
            self.aggregators.append(graphmodule.BiCombiner(input_size, output_size, dropout=self.mess_dropout[i], act=torch.nn.LeakyReLU()))
        self.NGCFNet = graphmodule.LightGCNNet_dglnn(self.aggregators, normalize=2, mess_norm='left')
    ...
    def update_encoders(self):
        ...
        if self.node_dropout != None: 
            ...
            adj_mat = self.sparseDropout(adj_mat)
        else:
