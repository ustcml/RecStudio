...
class SoftmaxApprSamplerUniform(Sampler):
    ...
    def __init__(self, num_items, num_clusters):
        super(SoftmaxApprSamplerUniform, self).__init__(num_items)
        self.K = num_clusters
...
class SoftmaxApprSamplerPop(SoftmaxApprSamplerUniform):
    ...
    """
    def __init__(self, pop_count, num_cluster, mode=1):
        super(SoftmaxApprSamplerPop, self).__init__(pop_count.shape[0], num_cluster)
        if mode == 0:
    ...
    def update(self, item_embs, max_iter=30):
        cd0m, cd1m = super(SoftmaxApprSamplerPop, self).update(item_embs, max_iter)
        self.wkk = cd0m.T @ (cd1m * self.pop_count.view(-1, 1))
        #self.p = torch.from_numpy(np.insert(pop_count, 0, 1.0)) # this is similar, to avoid log 0 !!! in case of zero padding 
        self.p = torch.cat([torch.tensor(1.0), self.pop_count], dim=0) # this is similar, to avoid log 0 !!! in case of zero padding 
        self.cp = self.pop_count[self.indices]
